...
:root{
  ...
  --ink:#16211F; --ink-2:#3A4744; --muted:#636F6B; --line:#D3DAD7; --line-2:#E4E9E7;
...
.chip{background:var(--surface);border:1px solid var(--line);border-radius:7px;padding:.62rem .7rem;min-height:44px;
...
.chip .mk{font-size:.85rem;line-height:1;flex:none}
.chip .mk.ranking_modelo{color:var(--ok)}
.chip .mk.regra_simples{color:var(--warn)}
.chip .mk.abster{color:var(--muted)}
...
.legend-uf i{font-style:normal;margin-right:.35rem}
.legend-uf i.ranking_modelo{color:var(--ok)}
.legend-uf i.regra_simples{color:var(--warn)}
.legend-uf i.abster{color:var(--muted)}
...
.trust{display:flex;flex-wrap:wrap;align-items:flex-start;gap:.5rem;padding:.7rem .95rem;border-radius:8px;font-size:.9rem;margin-bottom:1.1rem;line-height:1.45}
...
.cross-hint{background:var(--surface-2);border:1px solid var(--line);border-radius:8px;padding:.65rem .9rem;margin-bottom:.8rem;font-size:.86rem;color:var(--ink-2);line-height:1.6}
.cross-hint button{background:none;border:none;padding:0;margin:0 .1rem;color:var(--primary);font-family:var(--mono);font-size:.86rem;font-weight:600;cursor:pointer;text-decoration:underline}

.scroll-hint{display:none;font-family:var(--mono);font-size:.72rem;color:var(--muted);margin:0 0 .5rem}
...
.sort-ic{display:inline-block;min-width:.9em;font-family:var(--mono)}
...
.riskfill{display:block;height:100%;border-radius:99px}
...
<div class="wrap">
  ...
  <div class="picker">
    ...
    <div class="legend-uf">
      <span><i class="ranking_modelo" aria-hidden="true">✓</i>ranking do modelo (venceu o backtest)</span>
      <span><i class="regra_simples" aria-hidden="true">⚠</i>regra simples (modelo perdeu)</span>
      <span><i class="abster" aria-hidden="true">≈</i>abstenção (inconclusivo)</span>
    </div>
  </div>
  ...
  <div id="crossHint"></div>
  <p class="scroll-hint">Deslize a tabela para o lado para ver Taxa 2024, Meta e Taxa 2025 &rarr;</p>

  <div class="tbl-wrap">
    <table>
      <thead><tr>
        <th>#</th>
        <th class="sortable" data-idx="1">Município<span class="sort-ic"></span></th>
        <th id="thOrd" class="sortable" data-idx="2"><span class="lbl">Risco previsto</span><span class="sort-ic"></span></th>
        <th class="sortable" data-idx="6">Resultado real<span class="sort-ic"></span></th>
        <th class="sortable" data-idx="3" style="text-align:right">Taxa 2024<span class="sort-ic"></span></th>
        <th class="sortable" data-idx="4" style="text-align:right">Meta 2025<span class="sort-ic"></span></th>
        <th class="sortable" data-idx="5" style="text-align:right">Taxa 2025<span class="sort-ic"></span></th>
      </tr></thead>
      <tbody id="rows"></tbody>
    </table>
  </div>
...
const GLYPH = {ranking_modelo:"✓", regra_simples:"⚠", abster:"≈"};
const USO_LABEL = {
  ranking_modelo: "ranking do modelo, venceu o backtest",
  regra_simples: "regra simples, modelo perdeu",
  abster: "abstenção, inconclusivo"
};
...
function montarChips(){
  el("chips").innerHTML = ufs.map(uf =>
    `<button class="chip" data-uf="${uf}" aria-pressed="${uf===atual}" aria-label="${uf}, ${USO_LABEL[DATA.ufs[uf].uso]}">` +
    `<span class="mk ${DATA.ufs[uf].uso}" aria-hidden="true">${GLYPH[DATA.ufs[uf].uso]}</span>${uf}</button>`).join("");
  el("chips").querySelectorAll(".chip").forEach(b =>
    b.addEventListener("click", () => {
      atual = b.dataset.uf; filtro=""; el("search").value=""; sortCol=null; sortDir=1; render();
    }));
}

function montarSortHandlers(){
...
function ordenar(linhas){
  if (sortCol === null) return linhas;
  const idx = sortCol;
  return [...linhas].sort((a, b) => {
    const x = a[idx], y = b[idx];
    if (typeof x === "string") return sortDir * x.localeCompare(y, "pt-BR");
...
function render(){
  ...
  el("thOrd").querySelector(".lbl").textContent = d.uso === "regra_simples" ? "Score do modelo (ref.)" : "Risco previsto";
  ...
  let linhas = d.m.filter(r => !termo || r[1].toLowerCase().includes(termo));
  ...
  montarCrossHint(termo, linhas.length);
...
function montarCrossHint(termo, encontradosNaUf){
  if (!termo || encontradosNaUf > 0) { el("crossHint").innerHTML = ""; return; }

  const achados = [];
  for (const uf of ufs) {
    if (uf === atual) continue;
    for (const r of DATA.ufs[uf].m) {
      if (r[1].toLowerCase().includes(termo)) achados.push({ uf, nome: r[1] });
    }
  }

  if (!achados.length) {
    el("crossHint").innerHTML = `<div class="cross-hint">Nenhum município encontrado em nenhum estado para "${filtro.trim()}".</div>`;
    return;
  }

  el("crossHint").innerHTML = `<div class="cross-hint">Não encontrado em ${DATA.ufs[atual].nome}. Encontrado em outro estado: ` +
    achados.slice(0, 8).map(a => `<button type="button" data-uf="${a.uf}" data-nome="${a.nome}">${a.nome} (${a.uf})</button>`).join(", ") +
    (achados.length > 8 ? "…" : "") + `</div>`;
  el("crossHint").querySelectorAll("button").forEach(b => b.addEventListener("click", () => {
    atual = b.dataset.uf; filtro = b.dataset.nome; el("search").value = filtro; sortCol=null; sortDir=1; render();
  }));
}
